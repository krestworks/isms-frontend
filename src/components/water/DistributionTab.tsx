...
export function DistributionTab() {
  ...
  const exportColumns: ExportColumn<ApiWaterDistribution>[] = [
    ...
    { label: "Loaded (L)",      value: d => d.litresLoaded, total: rows => rows.reduce((sum, d) => sum + d.litresLoaded, 0) },
    { label: "Delivered (L)",   value: d => d.litresDelivered, total: rows => rows.reduce((sum, d) => sum + d.litresDelivered, 0) },
    { label: "Variance (L)",    value: d => d.variance, total: rows => rows.reduce((sum, d) => sum + d.variance, 0) },
