...
export function RefillsTab() {
  ...
  const exportColumns: ExportColumn<ApiLpgRefill>[] = [
    ...
    { label: "Qty",            value: r => r.quantity, total: rows => rows.reduce((sum, r) => sum + r.quantity, 0) },
    { label: "Cost/Unit (Ksh)",value: r => r.costPerUnit },
    { label: "Total (Ksh)",    value: r => r.totalCost, total: rows => rows.reduce((sum, r) => sum + r.totalCost, 0) },
