...
export function ReconciliationTab() {
  ...
  const exportColumns: ExportColumn<ApiFuelReconciliation>[] = [
    ...
    { label: "Deliveries (L)",    value: r => r.deliveries, total: rows => rows.reduce((sum, r) => sum + r.deliveries, 0) },
    { label: "Expected Sales (L)",value: r => r.expectedSales, total: rows => rows.reduce((sum, r) => sum + r.expectedSales, 0) },
    { label: "Actual Sales (L)",  value: r => r.actualSales, total: rows => rows.reduce((sum, r) => sum + r.actualSales, 0) },
    { label: "Closing Expected (L)", value: r => r.closingStockExpected },
    { label: "Closing Actual (L)",value: r => r.closingStockActual },
    { label: "Variance (L)",      value: r => r.variance, total: rows => rows.reduce((sum, r) => sum + r.variance, 0) },
